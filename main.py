...
# # main.py
# import os
# # Limit OpenMP threads to avoid segmentation faults in libomp
# os.environ['OMP_NUM_THREADS'] = '1'
...
#     # Load mobile detection model only if enabled
#     if ENABLE_MOBILE_DETECTION:
#         mobile_net = load_mobile_net()
#         mobile_net.to('cpu')
#         mobile_net.eval()
...
#                 if recent_emotions:
#                     from collections import Counter
#                     freq = Counter(recent_emotions)
#                     smoothed_emotion = freq.most_common(1)[0][0]
#                     raw_emotions[0] = (smoothed_emotion, score)
...
#                         startX, startY, endX, endY = phone_box
#                         phone_center = ((startX + endX) // 2, (startY + endY) // 2)
#                         for box, name in zip(smoothed_face_locations, face_names):
#                             if name != "Unknown" and point_in_box(phone_center, box):
#                                 # Normalize the name: replace underscores with spaces.
#                                 normalized_name = name.replace("_", " ")
#                                 alert_message = f"Please put your phone away, {normalized_name}"
#                                 print(f"[DEBUG] Alert triggered: {alert_message}")
#                                 # Use Popen to invoke the TTS command without waiting
#                                 subprocess.Popen(["/usr/bin/say", alert_message])
#                                 break
...
# def tts_alert(message):
#     # Use subprocess.Popen (non-blocking) in a separate thread to play TTS
#     try:
#         # Using /usr/bin/say; adjust if needed.
#         subprocess.Popen(["/usr/bin/say", message])
#     except Exception as e:
#         print(f"[ERROR] TTS alert failed: {e}")
...
#                                 alert_message = f"Please put your phone away, {normalized_name}"
#                                 print(f"[DEBUG] Alert triggered: {alert_message}")
#                                 # Spawn a thread to play the TTS alert
#                                 threading.Thread(target=tts_alert, args=(alert_message,), daemon=True).start()
#                                 break
...
# def tts_alert(message):
#     """
#     Generates an alert audio using gTTS and plays it using afplay.
#     """
#     try:
...
#         # Wait a few seconds to allow the sound to play
#         import time
#         time.sleep(3)
#     except Exception as e:
#         print(f"[ERROR] TTS alert failed: {e}")
